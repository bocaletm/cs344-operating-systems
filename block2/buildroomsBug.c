 ...
 * if a room is connected to another
 * ***********************/
int hasConnection(int** connections) {
  int isConnected = 0;
  return isConnected;
}
/**************************
...
int addConnections(char* dirName,int* createdRooms) {
    ...
    /* the number of connections for each room
     * followed by the indices of the rooms it's connected to */
    int connections[NUM_ROOMS][ROOMS_TO_CREATE];
    int i;
    int j;
    ...
    for (i = 0; i < ROOMS_TO_CREATE; i++) {
      idx = 0;
      while (connections[createdRooms[i]][0] < 3) {
        if (createdRooms[i] != randomRoomsPtr[idx] && connections[randomRoomsPtr[idx]][0] < 6) {
          /*find out if connection is duplicate*/
          for (k = 1; k < ROOMS_TO_CREATE; k++) {
            if (connections[createdRooms[i]][k] == randomRoomsPtr[idx]) {
              printf("%s and %s are already connected\n",ROOMS[createdRooms[i]],ROOMS[randomRoomsPtr[idx]]);
            }
          }
          
        /*do not add duplicate room connections*/
          /*keep count and track connections*/
          connections[createdRooms[i]][0]++;
          newConnIdx = connections[createdRooms[i]][0];
          connections[createdRooms[i]][newConnIdx] = randomRoomsPtr[idx];
          
          connections[randomRoomsPtr[idx]][0]++;
          newConnIdx = connections[randomRoomsPtr[idx]][0];
          connections[randomRoomsPtr[idx]][newConnIdx] = createdRooms[i];

          /*write connections to files*/
          connect(createdRooms[i],randomRoomsPtr[idx],connections[createdRooms[i]][0],connections[randomRoomsPtr[idx]][0],dirName);
        }
        idx++; 
      }
    }
